    ...
    <div class='flex bg-slate-800 rounded-md items-center border-transparent' onMouseEnter={() => setHover(true)} onMouseLeave={() => setHover(false)}>
        ...
        <div class='flex flex-col gap-0'>
            ...
            {/* for loop was not working for some reason */}
                ...
                <Icon path={mdiStar} size={0.5} class={props.skill.stars > 2 ? 'text-amber-500' : 'text-gray-500'} />
                <Icon path={mdiStar} size={0.5} class={props.skill.stars > 3 ? 'text-amber-500' : 'text-gray-500'} />
                <Icon path={mdiStar} size={0.5} class={props.skill.stars > 4 ? 'text-amber-500' : 'text-gray-500'} />
            </div>: <></>}
        </div>
