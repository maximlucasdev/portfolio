...
export const commands = [
  {
    ...
    Response: (props: { scroll: () => {} }) => {
      ...
      return (
        <h1>
          <p className="gradient-text w-max">
            ========= List of commands =========
          </p>
          <p>
            • help{" "}
            <span className="text-gray-400">- Get a list of commands</span>
          </p>
          <p>
            • bootmenu{" "}
            <span className="text-gray-400">- Enter the boot menu</span>
          </p>
          <p>
            • neofetch{" "}
            <span className="text-gray-400">- Get my main PC specs</span>
          </p>
          <p>
            • htop <span className="text-gray-400">- Show a fake htop</span>
          </p>
          <p>
            • about
            <span className="text-gray-400">- About this portfolio</span>
          </p>
          <p>
            • clear
  ...
  },
  {
    ...
    Response: (props: { scroll: () => {} }) => {
      ...
      if (pic === "")
        return (
          <p>
            <Spinner /> Fetching a new Pepsi picture...
          </p>
        );
      return (
        <>
          <img src={pic} alt="pepsi" onLoad={() => props.scroll()} />
          <p>🐈 Pepsi Mode {pepsimode.value ? "enabled" : "disabled"}. Meow!</p>
        </>
      );
    },
  },
      ...
      return (
        <div className="flex flex-row gap-2">
          ...
          <p className="text-gray-300 w-1/2">
            <span className="gradient-text">OS:</span> PepsiOS 1.1.2 x64
            <br />
            <span className="gradient-text">Host:</span> xshadow
            <br />
            <span className="gradient-text">Uptime:</span> {uptime}
            <br />
            <span className="gradient-text">Resolution:</span>{" "}
            {window.innerWidth}x{window.innerHeight}
            <br />
            <span className="gradient-text">CPU: </span> AMD Ryzen™ 7 7700 (16)
            @ 5.3GHz
            <br />
            <span className="gradient-text">GPU:</span> NVIDIA GeForce RTX™ 4060
            <br />
            <span className="gradient-text">Memory:</span> 31864MB DDR5
            <br />
          </p>
        </div>
      );
    ...
    Response: () => {
      ...
      useEffect(() => {
        const interval = setInterval(() => {
          ...
          setProcess(
            POSSIBLE_PROCESSES[
              Math.floor(Math.random() * POSSIBLE_PROCESSES.length)
            ]
          );
        }, 2000);
      ...
      return (
        <>
            ...
            {cpus.map((percentage, index) => {
              return (
                <p class="gradient-text">
                  {index}[{"|".repeat(Math.floor((percentage * 20) / 100))}
                  <span class="invisible">
                    {"|".repeat(20 - Math.floor((percentage * 20) / 100))}
                  </span>
                  {percentage < 10 ? " " : ""}
                  {percentage}%]
                </p>
          ...
          <div class="flex flex-col">
            <p class="gradient-text">
              Mem[|||<span class="invisible">|||||||||||||||||</span>6G/32G]
            </p>
            <p class="gradient-text">
              Swp[<span class="invisible">|||||||||||||||||||||</span>0K/2G]
            </p>
          </div>
          <div class="flex gap-2 h-5 text-black mt-3">
            <div class="px-2 bg-orange-500">
              <p>Main</p>
            </div>
            <div class="px-2 bg-orange-500 opacity-50">
              <p>I/O</p>
            </div>
          </div>
      ...
      return (
        <p>
          <span className="gradient-text font-bold">
            apps pepsi-the-cat sideprojects
          </span>{" "}
          Portfolio.tsx
        </p>
      );
  ...
  },
  {
    ...
    Response: (props: { scroll: () => {} }) => {
      ...
      return <p>This is not a real terminal!</p>;
    },
  },
];
