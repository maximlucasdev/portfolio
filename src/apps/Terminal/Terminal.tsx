...
export default function Terminal() {
  ...
  return (
    <div
      className="w-full h-full md:h-96 rounded-b-md"
      onClick={() => focusInput()}
    >
      <div className="flex flex-col justify-end">
        <div
          className="font-mono w-full h-full md:h-96 p-3 text-white overflow-y-scroll noscrollbar"
          style={{
            height: window.innerWidth < 768 ? window.innerHeight - 100 : 385,
          }}
          id={termId}
        >
          <p className="text-gray-400 mb-5">
            <span class="gradient-text">Welcome to PortfoliOS 1.1.2 (x64)</span>
            <br />
            <br />
            * Website: https://maximlucas.dev
            <br />* Support: contact@maximlucas.dev
          </p>
          <p className="text-white/50 text-xs">
            (i) {t("app.terminal.firsthint")}
          </p>
          {history.map((value) => {
            ...
            return (
              <Component
                ...
                scroll={() => {
                  setTimeout(
                    () =>
                      (document.getElementById(termId).scrollTop =
                        document.getElementById(termId).scrollHeight),
                    50
                  );
                }}
              />
          ...
          <p>
            ...
            <input
              ...
              style={{
                width: inputData.length === 0 ? 1 : `${inputData.length}ch`,
              }}
              placeholder=""
              value={inputData}
              onChange={(value) => {
              ...
              onKeyDown={(key) => {
                switch (
                  key.keyCode // Had to use deprecated keycode to prevent compatibility issues
                ) {
                  case 13:
                    ...
                    const command = commands.filter(
                      (value) => value.name === inputData
                    )[0];
                    const comp = () => {
                      return <p>visitor@maxim:~$ {inputData}</p>;
                    };
                    if (!command) {
                      setHistory([
                        ...history,
